...
def roll_dice(num_rolls, dice=six_sided_dice, who='Boss Hogg', ones_lose=True):
    ...
    if 1 in roll_result and ones_lose: return 1
...
def play(strategy0, strategy1):
    ...
    global goal
    while s0 < goal and s1 < goal:
        current_player_rolls = min(strategy0(s0, s1), num_allowed_dice(s0, s1))
        s0 = s0 + take_turn(current_player_rolls, s1, select_dice(s0, s1), name(who), s0 != 49)
        who = other(who)
        s0, s1 = s1, s0
