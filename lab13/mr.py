...
HADOOP_HOME = '/home/ff/cs61a/projects/hadoop/hadoop'
STREAMING = HADOOP_HOME + '/contrib/streaming/hadoop-streaming-1.0.2.jar'
...
def run_general(mapper, reducer, input_dir, output_dir, cachefile=None):
    """Run a MapReduction."""
    ...
    if cachefile:
        cmd += ' -cacheFile {0}'.format(cachefile)
...
def run(mapper, reducer, input_dir, output_dir):
    return run_general(mapper,reducer, input_dir, output_dir)

@command
def run_with_cache(mapper, reducer, input_dir, output_dir, cachefile):
    return run_general(mapper,reducer, input_dir, output_dir, cachefile)
