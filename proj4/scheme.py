...
def apply_primitive(procedure, args, env):
    ...
    """
    "*** YOUR CODE HERE ***"
    list_of_arguments = list(args)  
    if procedure.use_env:
        list_of_arguments.append(env)
    try:
        return procedure.fn(*list_of_arguments)
    except TypeError as someError:
        raise SchemeError(someError)
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) == len(vals):
            for f in range(len(vals)):
                frame.bindings[formals[f]] = vals[f]
        else: raise SchemeError("too many or two few variables")
        return frame
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        # convert (define (f x y) (+ x y)) to 
        # (define f (lambda (x y) (+ x y)))
        fn = vals[0].first
        if scheme_symbolp(fn):
            args =  Pair(vals[0].second, vals.second)
            lambda_exp = Pair("lambda", args)
            env.define(fn, scheme_eval(lambda_exp, env))
            return fn
        else:
...
def do_if_form(vals, env):
    ...
    check_form(vals, 2, 3)
    "*** YOUR CODE HERE ***"
    # since (if (+ 1 2) 3) returns 3 in stk, we should test if 
    # it is not false. This can be done by scheme_true
    predicate = scheme_eval(vals[0], env)
    if scheme_true(predicate):
        return vals[1]
    elif scheme_false(predicate):
        try:
            return vals[2]
        except IndexError:
            return okay
...
def do_begin_form(vals, env):
    ...
    #     return_value = scheme_eval(vals[i], env)
    # return return_value  
    # this is wrong for (begin 30 'hello)
...
def check_formals(formals):
    ...
    list_of_arguments = []
    for i in formals:
        if i in list_of_arguments:
            raise SchemeError('repeated symbols: '+ str(formals))
        elif not scheme_symbolp(i):
            raise SchemeError('Non-symbol: '+str(i))
