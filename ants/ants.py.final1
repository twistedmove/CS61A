...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        location = self.place
        index = 0
        while location.entrance != None or location != hive:
            if len(location.bees)> 0 and index >= self.min_range and index <= self.max_range:
...
class BodyguardAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor - amount <= 0:
            # make the location of self contain self.ant
            self_place = self.place
            contained_ant = self.ant
            Place.remove_insect(self.place, self)
            Place.add_insect(self_place, contained_ant)
